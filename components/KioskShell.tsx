...
export default function KioskShell() {
  ...
  return (
    <div className="v3-shell" onPointerDown={resetIdle}>
      ...
      {notProvisionedAlert && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center" style={{ backgroundColor: "rgba(0,0,0,0.5)" }} onClick={() => setNotProvisionedAlert(false)}>
      ...
      {/* Floor picker */}
      {floorPicker && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center" style={{ backgroundColor: "rgba(0,0,0,0.5)" }} onClick={() => setFloorPicker(null)}>
          <div className="bg-white rounded-2xl max-w-xs w-full mx-6 overflow-hidden" onClick={e => e.stopPropagation()}>
      ...
      <div className="v3-sidebar">
        ...
        <div className="v3-brand">
          <p className="v3-brand-sub">Campus Directory</p>
          <p className="v3-brand-name">Sunway<br />University</p>
        </div>

        <div className="v3-nav-sep" />
        ...
        <nav className="v3-nav">
          {TAB_LABELS.map((label, i) => (
            <button
              key={label}
              className={`v3-nav-btn${tab === i && !showResults ? " active" : ""}`}
              onClick={() => { handleTabChange(i); }}
            >
              <div className="v3-nav-indicator" />
              <span className="v3-nav-label">{label}</span>
            </button>
          ))}
        </nav>
        ...
        <div className="v3-emergency">
          <p className="v3-emergency-lbl">Emergency Hotline</p>
          <p className="v3-emergency-num">+603-7491 8777</p>
          <p className="v3-version">v1.0 Build #14</p>
        </div>
      </div>
      ...
      <div className="v3-panel">
        {/* Search bar */}
        <div className="v3-search-row">
          <div className="v3-search-wrap">
            <svg className="v3-search-icon" width="18" height="18" viewBox="0 0 24 24" fill="none">
            ...
            />
            {query.length > 0 && (
              <button onClick={handleClear} className="v3-clear">Clear</button>
            )}
          </div>
        </div>
