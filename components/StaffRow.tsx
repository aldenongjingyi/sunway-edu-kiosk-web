    ...
    <div className="row-press" onClick={() => onSelect(staff)}>
      <div className="flex items-start gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {staff.locationTitle && (
            <p className="text-[14px] text-[#3c3c43] mt-0.5">{staff.locationTitle}</p>
          )}
          {(staff.buildingName || staff.floorLabel) && (
            <p className="text-[13px] text-[#6b6b6b] mt-0.5">
              {[staff.buildingName, staff.floorLabel].filter(Boolean).join(" · ")}
            </p>
          )}
